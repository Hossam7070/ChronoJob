...
const CreateJob = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Set the location to the uploads directory
      setFormData({
        ...formData,
        data_source: { 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate file upload for internal file type
      ...
      // Upload file if it's a file source
              ...
              <div>
                ...
                {selectedFile && (
                  <p className="mt-2 text-xs text-gray-500">
                    File will be saved to: {formData.data_source.location}
                  </p>
                )}
              </div>
            )}
